...
SELECT
  ...
  COALESCE(
    statement.sql_text, thread_a.PROCESSLIST_info
  ) AS sql_text,
  ...
  IF(
    waits_a.thread_id IS NULL,
    "other",
    COALESCE(
      IF(
        thread_a.processlist_state = "User sleep",
        "User sleep",
        IF(
          waits_a.event_id = waits_a.end_event_id,
          "CPU", waits_a.event_name
        )
      ),
      "CPU"
    )
