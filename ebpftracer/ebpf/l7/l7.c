...
// 4 - 139723741988784
static inline __attribute__((__always_inline__))
int trace_enter_write(struct trace_event_raw_sys_exit_rw__stub_new* ctx) {
    ...
    // if (is_mysql_query(ctx->buf, ctx->count, &req->request_type)) {
    ...
    // }
    bpf_probe_read(req->payload, MAX_PAYLOAD_SIZE, (char *)ctx->ubuf);
...
int sys_enter_read(struct trace_event_raw_sys_exit_rw__stub_new* ctx) {
    return trace_enter_write(ctx);
}


// SEC("kprobe/tcp_recvmsg")
// int tcp_recvmsg(void* ctx) {
//     __u64 id = bpf_get_current_pid_tgid();
//     if (id != 3478){
//         return 0;
//     }
...
//     //     req->ns = bpf_ktime_get_ns();
//     // }
//     // bpf_probe_read(req->payload, MAX_PAYLOAD_SIZE, (char *)ctx->buf);
//     // bpf_map_update_elem(&active_l7_requests, &k, req, BPF_ANY);
//     bpf_perf_event_output(ctx, &l7_events, BPF_F_CURRENT_CPU, req, sizeof(*req));
//     return 0;
// }